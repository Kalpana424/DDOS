...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "ꜰʀᴇᴇ nahi hai Buy now:- @rishabgupta01 ❄."
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- @rishabgupta01 KA KALA JADU"
    bot.reply_to(message, response)
...
def welcome_start(message):
    ...
    response = f'''❄️ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴘʀᴇᴍɪᴜᴍ ᴅᴅᴏs ʙᴏᴛ, {user_name}! BGMI KI MAA CHOD DENGA . ᴛᴏ ɢᴇᴛ ᴀᴄᴄᴇss.
